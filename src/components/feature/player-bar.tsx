    ...
    <div class="border-t border-white/10 bg-neutral-950/90 backdrop-blur">
      <div class="mx-auto flex w-full max-w-6xl flex-col gap-3 px-4 py-4 sm:px-6">
        <div class="flex items-center gap-4">
          <div class="h-11 w-11 overflow-hidden rounded-2xl theme-gradient-bg">
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-semibold text-white">
              {player.current?.title ?? "暂无播放"}
            </p>
            <p class="truncate text-xs text-neutral-400">
              {player.current?.artist
                ? `${player.current.artist} · ${player.current.album ?? ""}`
                : "选择一首歌曲"}
              {player.current?.sourceType
                ? ` · ${player.current.sourceType === "local" ? "本地" : "在线"}`
                : ""}
            </p>
          </div>
